...
function HomePage() {
  ...
  //         useChatStore.getState().resetOnAuthChange();
  //       }
  //     }
  //   );
  //   return () => unsubscribe();
                ...
                <div className="flex-1 overflow-y-auto">
                  {isMessagesLoading ? (
                    ...
                    <div className="p-6 space-y-6">
                      {messages.length > 0 ? (
                        messages.map((msg) => (
                          <MessageItem key={msg._id || msg.id} msg={msg} theme={theme} selUser={selectedUser} messageEndRef={messageEndRef}/>
                        ))
                      ) : (
...
// MessageItem and EmptyChatState components remain exactly the same
const MessageItem = ({ msg, theme, selUser,messageEndRef }) => {
  const { authUser } = useAuthStore.getState();
  const isCurrentUser = msg.sender === "You" || msg.senderId === authUser?._id;

  return (
    <div className={`flex items-start gap-2 ${isCurrentUser ? "justify-end" : ""}`} ref={messageEndRef}>
      {!isCurrentUser && (
